...
public class InMemoryRepo<T> : IRepo<T> where T : notnull, IEntity
{
    ...
    public Task<T> DeleteAsync(Guid id, CancellationToken ct = default)
    {
        ...
        if (!removed || json == null) throw new KeyNotFoundException($"Entity with id {id} not found");
        return Task.FromResult(JsonConvert.DeserializeObject<T>(json) ?? throw new JsonException($"Stored JSON for entity {id} did not deserialize"));
    }
