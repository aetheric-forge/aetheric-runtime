...
public class RepoTests
{
    ...
    public async Task Delete_Removes_Item_And_Returns_True_When_Deleted(Func<IRepo<TestModels.TestMessage>> factory)
    {
        ...
        Assert.Throws<KeyNotFoundException>(() => repo.GetAsync(message.Id).GetAwaiter().GetResult());
    }

    [Theory]
    [MemberData(nameof(Cases))]
    public async Task Delete_Returns_False_When_Item_Does_Not_Exist(Func<IRepo<TestModels.TestMessage>> factory)
    {
        var repo = factory();
        await repo.ClearAsync();

        var deleted = await repo.DeleteAsync(Guid.Empty);
